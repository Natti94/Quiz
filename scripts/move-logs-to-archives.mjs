...
// Usage: INFRA_GIT_URL=git@github.com:org/infra.git node scripts/move-logs-to-archives.mjs [srcFolderInInfra]
// Options:
//   --commit    commit & push copied files into the local repo (off by default)
//   --force     overwrite existing files in archives
//   --dry-run   show what will be copied, do not modify files
...
try {
  ...
  // Source path inside infra clone
  ...
  for (const f of files) {
    if (f.startsWith('tag-backup-') || f.endsWith('-sha.txt')) {
      ...
      let shouldCopy = true;
      try {
        const [srcBuf, destBuf] = await Promise.all([fs.readFile(src), fs.readFile(dest).catch(() => null)]);
        if (destBuf && Buffer.compare(srcBuf, destBuf) === 0) {
          shouldCopy = false; // same file, skip
        }
      } catch (e) {
  ...
  if (copies.length === 0) {
    ...
    if (commit) {
      try {
        execSync('git add ' + copies.map(p => JSON.stringify(path.relative(process.cwd(), p))).join(' '), { stdio: 'inherit' });
        execSync('git commit -m "chore(logs): import tag backups from infra"', { stdio: 'inherit' });
      ...
      console.log('Dry run or --commit was not specified; not committing copied files.');
    }
  }
} catch (err) {
